...
class CarEnv_04_Ramp_Merge:
    ...
    def __init__(self):
        ...
        settings.max_substep_delta_time = 0.03  # fixed_delta_seconds <= max_substep_delta_time * max_substeps
    ...
    def global_routing(self):
        ...
        for wp in current_route:
            lanepoint = Lanepoint()
            lanepoint.position.x = wp[0].transform.location.x + 2.0 #FIXME: It should not do that!
...
start_point_02.location.x = -47
start_point_02.location.y = 186
...
class CarEnv_03_Ramp_Merge:
    ...
    def __init__(self):
        ...
        settings.fixed_delta_seconds = 0.05 # Warning: When change simulator, the delta_t in controller should also be change.
        settings.substepping = True
        ...
        for i in range(0,10):
            for j in range(0,10):
                for k in range (0,5):
                    ...
                    useless_transform.location.x = -8 
                    useless_transform.location.y = 175
                    useless_transform.location.z = 1
                    useless_transform.rotation.pitch = 0
                    useless_transform.rotation.yaw = 90
                    useless_transform.rotation.roll = 0
                    spawn_vehicles.append(useless_transform)

                    useless_transform_2 = Transform()
                    useless_transform_2.location.x = -13 
                    useless_transform_2.location.y = 193
                    useless_transform_2.location.z = 1
                    useless_transform_2.rotation.pitch = 0
                    useless_transform_2.rotation.yaw = -180
    ...
    def spawn_fixed_veh(self):
        ...
        batch = []
        print("Case_id",self.case_id, len(self.case_list[self.case_id - 1]))
        ...
        for vehicle in vehicle_list:  
            ...
            self.tm.ignore_walkers_percentage(vehicle, 0)
            self.tm.auto_lane_change(vehicle, True)
            self.tm.vehicle_percentage_speed_difference(vehicle, -100)
